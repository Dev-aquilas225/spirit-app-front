...
export default function PrayerProgramScreen() {
  ...
  // ─── Header Universel Réparé ───────────────────────────────────────────────
  const HeaderBlock = (
    <View style={[s.header, { backgroundColor: colors.deepBlue ?? "#1A1A3E" }]}>
      <View style={s.headerRow}>
        ...
        <View style={{ flex: 1 }}>
          <View style={s.headerTitleRow}>
            <AppIcon icon={Heart} size={18} color="#C9A84C" strokeWidth={2.6} />
        ...
        {/* Afficher le bouton historique uniquement si on est dans le chat */}
        {view === "chat" && (
          <TouchableOpacity
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {preview ? truncateText(preview, 45) : "Session de prière"}
                    </Text>
                    <Text
                      style={{
  ...
  return (
    ...
    >
      <FadeInView style={{ flex: 1 }}>
        ...
        {isLoading ? (
          <LoadingSpinner fullScreen message="Connexion spirituelle..." />
        ) : messages.length === 0 ? (
          <ScrollView 
          ...
          >
            ...
            <Text style={[s.emptyTitle, { color: colors.text }]}>
              {firstName ? `Paix et bénédictions, ${firstName}` : "Paix et bénédictions"}
            </Text>
            <Text style={[s.emptySub, { color: colors.textSecondary }]}>
              Le Prophète Georges vous accompagne sur votre chemin de foi et d'intercession prophétique.{"\n\n"}Quelle est votre intention ou le fardeau de votre cœur aujourd'hui ?
            </Text>
            
            {/* Boîtes de Suggestions Spirituelles */}
        ...
        {/* Animation d'attente prophétique */}
        {isSending && (
          <View style={[s.typing, { backgroundColor: colors.surface, borderTopWidth: 0.5, borderTopColor: colors.border }]}>
            <View style={s.typingRow}>
              <ActivityIndicator size="small" color="#C9A84C" style={{ marginRight: 4 }} />
              <Text style={{ color: colors.textSecondary, fontSize: 12, fontStyle: "italic", fontWeight: "500" }}>
                Le Prophète Georges intercède pour vous…
              </Text>
            </View>
          </View>
        )}
...
const s = StyleSheet.create({
  ...
  bulletPoint: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#C9A84C" },
  typing: { paddingHorizontal: 16, paddingVertical: 10 },
