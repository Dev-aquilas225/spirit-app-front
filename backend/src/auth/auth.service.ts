...
export class AuthService {
  ...
  async sendMagicLink(email: string) {
    ...
    const link = `${this.cfg.get('APP_BASE_URL', 'https://oracle-plus.online')}/callback?token=${token}`;
  ...
  async verifyMagicLink(token: string) {
    ...
    // Fallback: find by token via direct query
    const { DataSource } = require('typeorm');
    return { message: 'Use /auth/google instead' };
  }
